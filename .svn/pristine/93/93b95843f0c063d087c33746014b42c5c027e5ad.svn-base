    ...
    Try
      ...
      Begin
        ...
        begin
          ...
          ConexaoBanco.Param.ParamByName('MEMORECEBER').AsString := 'TITULAR DO CARTÃO: ' + CtReceber.TitularCartao;
...
Begin
  ...
  while not DataSet.Eof do
  begin
    if DataSet.FieldByName('SITUACAO').AsInteger = 1 then //Se o documento estiver em aberto ainda, so deverá exclui-lo
    begin
      ConexaoBanco.Clear;
      ConexaoBanco.ClearParam;
...
begin
  Try
    ...
    On E : Exception do
    Begin
      TCustomExcept.Show(e, 'Erro ao buscar extensão fatura', False);
...
begin
  ...
  Begin
    ...
    begin
      ...
      begin
        sMessage := 'Não é possível alterar o Sinal pois o Financeiro já está quitado.';
        Result := True;
      end
      else
        Result := False;
    ...
    begin
      sMessage := 'Não é possível alterar para aberta pois o sinal já está quitado.';
      Result := True;
    end
    else
    Begin
      ...
      begin
        sMessage := 'Não é possível alterar a O.S. pois já possui Financeiro quitado.';
        Result := True;
      end;
    End;
  End;
...
begin
  ...
  begin
    sMessage := 'Não existe esse plano de pagamento configurado para esta Forma de Pagamento.';
    Result := False;
    Exit;
  end
  else
  begin
    ...
    begin
      sMessage := 'Não Existem Parâmetros Configurados para esta Forma de Pagamento.';
      Result := False;
      Exit;
    end
    else
...
begin
  ...
  begin
    ...
    ConexaoBanco.Param.ParamByName('CODIGOFORMAPAG').AsInteger      := CtReceber.CodPlanoCartao;//se é 30,60 e 90
    ...
    ConexaoBanco.Param.ParamByName('MEMOPAGAR').AsString            := 'AUTORIZAÇÃO: ' + CtReceber.AutorizacaoOperadora;
    ConexaoBanco.DataSetFactory.PostDataSet(TConexao.Empresa, ConexaoBanco.Sql.Text, ConexaoBanco.Param);
